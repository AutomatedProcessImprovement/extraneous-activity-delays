...
def experimentation_real_life():
    ...
    with open("../outputs/real-life-evaluation/metrics.csv", "a") as output_file:
        output_file.write(
            "dataset,cycle_time_raw,cycle_time_direct_enhanced,cycle_time_hyperopt_enhanced,"
        ...
        print(
            "{},{},{},{},{},{}".format(
                ...
                len(
                    set(
                        [
                            ",".join(list(events[config.log_ids.activity]))
                            for key, events in log.groupby(config.log_ids.case)
                        ]
                    )
                ),
